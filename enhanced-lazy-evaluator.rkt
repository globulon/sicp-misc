        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           ...
           (list-of-operands (procedure-parameter-types procedure)
                             arguments 
                             env)
...
(define (delay-lazy arg env)
  (list 'lazy-thunk arg env))

(define (delay-lazy-mem arg env)
  (list 'memo-thunk arg env))

(define (make-make-operand type)
  (cond ((eq? 'lazy type)
         delay-lazy)
        ((eq? 'lazy-mem type)
         delay-lazy-mem)
...
(define (list-of-operands types args env)
  (if (empty? types)
      '()
      (let ((make-operand (make-make-operand (car types))))
        (cons (make-operand (car args) env)
              (list-of-operands (cdr types) (cdr args) env)))))
...
(define (arguments->pairs arguments)
  (define (iter-normalize args acc)
    (cond ((empty? args)
           acc)
          ((pair? (car args))
           (iter-normalize (cdr args) (cons (car args) acc)))
          (else
           (iter-normalize (cdr args) (cons (list (car args) 'eager) acc)))))
  (iter-normalize (reverse arguments) '()))

(define (make-procedure arguments body env)
  (list 'procedure (arguments->pairs arguments) body env))
